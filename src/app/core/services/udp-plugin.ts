...
export interface UdpPluginInterface {
  ...
  send(options: { socketId: number; address: string; port: number; data: string }): Promise<void>;
  close(options: { socketId: number }): Promise<void>;
  addListener(event: 'receive', handler: (event: { socketId: number; remoteAddress: string; remotePort: number; data: string }) => void): Promise<{ remove: () => void }>;
